...
function turnCamera(name: string, state: Enum.UserInputState, inputObj: InputObject) {
	...
	camera.CFrame = camera.CFrame.mul(CFrame.fromEulerAngles(math.rad(60), 0, 0))
		.mul(new CFrame(new Vector3(name === "turnCameraLeft" ? -100 : 100, 0, -100)))
		.mul(CFrame.fromEulerAngles(0, math.rad(name === "turnCameraLeft" ? -90 : 90), 0))
		.mul(CFrame.fromEulerAngles(math.rad(-60), 0, 0));
	task.wait(0.05);
}
