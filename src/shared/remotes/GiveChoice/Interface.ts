...
const GiveChoice = ReplicatedStorage.WaitForChild("TS").WaitForChild("remotes").WaitForChild("GiveChoice").WaitForChild("GiveChoice") as RemoteEvent
...
export function Server_GiveChoice_Sub(callback: (player: Player, selectedChoiceIndex: unknown) => void) {
    GiveChoice.OnServerEvent.Connect((plr, index) => { callback(plr, index) })
}